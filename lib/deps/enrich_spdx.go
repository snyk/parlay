...
func enrichSPDX(bom *spdx.Document, logger *zerolog.Logger) {
	...
	for i, pkg := range bom.Packages {
		...
		go func(pkg *spdx_2_3.Package, i int) {
			...
			// First try to find VCS external references
			...
			for _, ref := range pkg.PackageExternalReferences {
				if !foundVCS && ref.RefType == "purl" && ref.Locator != "" {
					url, err := GetRepoURLFromPackage(ref.Locator, logger)
					if err != nil {
						logger.Debug().
							Str("package", pkg.PackageName).
							Str("purl", ref.Locator).
							Err(err).
			...
			// If no repository URL found, skip this package
			if !foundVCS || repoURL == "" {
				logger.Debug().
					Str("package", pkg.PackageName).
