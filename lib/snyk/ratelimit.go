...
// Rate limit defaults to stay under Snyk's 160 req/sec limit
const (
...
func (r *rateLimiter) wait(ctx context.Context) error {
	// Proactive throttling via token bucket
	if err := r.tokenBucket.Wait(ctx); err != nil {
		return err
	}

	// Reactive backoff if we got a 429
	for {
		r.mu.Lock()
...
// parseRateLimitResetHeader parses the X-RateLimit-Reset header value as seconds.
